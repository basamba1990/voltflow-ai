...
// Fonction de normalisation des résultats (pour uniformiser Backend/Fallback)
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // --- 1. TENTATIVE BACKEND FASTAPI (avec timeout) ---
    ...
    // --- 2. FALLBACK (uniquement si backend échoue) ---
    let finalResults;
    if (backendResults) {
      finalResults = backendResults;
    } else {
      console.log(`[Simulate] Generating fallback mesh for simulation ${simId}`);
      ...
      // Upload VTK file to Supabase Storage
    ...
    // 3. SIMULATION PROGRESS (pour l'UI)
    ...
    for (const simStep of simulationSteps) {
      ...
      const waitTime = config.mesh_density === 'high' ? 3000 : 
                      config.mesh_density === 'medium' ? 2000 : 1000;
      await sleep(waitTime);
    }

    // 4. Save results to database
    console.log(`[Simulate] Saving results to database for ${simulationId}`);

...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!fileName || !fileData || !userId) {
      return new Response(
        JSON.stringify({ success: false, error: 'Missing required fields: fileName, fileData, userId' }),
        {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        }
    ...
    // CORRECTION CRITIQUE : Liste complète des extensions autorisées
    const allowedTypes = [
      'stl', 'step', 'stp', 'obj', 'iges', 'igs',
      'vtp', 'vti', 'ply', 'vtk' // Formats ajoutés
    ]
    ...
    if (!fileExtension || !allowedTypes.includes(fileExtension)) {
      return new Response(
        JSON.stringify({ 
          success: false, 
          error: `Invalid file type. Allowed: ${allowedTypes.join(', ')}. Received: ${fileExtension}` 
        }),
        {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    ...
    // Generate unique filename avec structure userId/...
    ...
    if (uploadError) {
      ...
      // Vérification spécifique pour erreur RLS
      if (uploadError.message.includes('violates row-level security policy')) {
        return new Response(
          JSON.stringify({ 
            success: false, 
            error: 'RLS Policy violation. Please ensure your RLS policies are correctly configured.',
            details: 'The upload path must follow the pattern: userId/filename.ext'
          }),
          {
            status: 403,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },

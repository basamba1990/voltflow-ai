...
Version: 2.9 - FIX VOXELIZATION & VTK OUTPUT
"""
...
def voxelize_stl(stl_url, nx, ny, nz):
    ...
    try:
        resp = requests.get(stl_url, timeout=30)
        if resp.status_code != 200:
            raise RuntimeError(f"Impossible de télécharger le STL : {stl_url} (Status: {resp.status_code})")
        ...
        # Utilisation de mesh.contains pour vérifier si chaque point est à l'intérieur
        # On utilise une méthode plus robuste pour la voxelisation
        grid = np.stack(np.meshgrid(x, y, z, indexing='ij'), axis=-1)
        points = grid.reshape(-1, 3)
        
        # trimesh.contains est gourmand en mémoire, on peut traiter par morceaux si nécessaire
        inside = mesh.contains(points).reshape((nx, ny, nz))
        return inside.astype(int)
    except Exception as e:
        logger.error(f"Erreur lors de la voxelisation: {str(e)}")
        # Retourne un masque plein par défaut en cas d'erreur pour ne pas bloquer
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(config_file, 'w') as f:
                ...
                if mask is not None:
                    ...
                    # Sauvegarde du masque en format plat (F-style) pour Fortran
                    np.savetxt(mask_file, mask.flatten(order='F'), fmt='%d')
                    f.write(f"{mask_file}\n")
                else:
                    f.write("0\n")
            ...
            if os.path.exists(vtk_src):
                ...
                logger.warning(f"Fichier VTK non généré par le solveur à {vtk_src}")
...
async def simulate(request: Request, sim_request: SimulationRequest):
    ...
    try:
        ...
        if cfg.geometry_type == "complex" and cfg.mesh_file:
            logger.info(f"Voxelisation demandée pour {cfg.mesh_file}")
        ...
        if vtk_url:
            # S'assurer que l'URL est correcte par rapport à l'hôte de la requête
            base_url = str(request.base_url).rstrip('/')
            vtk_url = base_url + vtk_url
        ...
        # On utilise des listes Python simples pour la compatibilité JSON
        advective_flux = [float(avg_temp * cfg.density * 0.01)] * min(mesh_points, 1000)
        diffusive_flux = [float(cfg.conductivity * 0.5)] * min(mesh_points, 1000)
        ...
        response = SimulationResponse(
            ...
            temperature_field={
                "values": [float(avg_temp)] * min(mesh_points, 1000)
            },
            energy_flux={
                "total": total_flux,

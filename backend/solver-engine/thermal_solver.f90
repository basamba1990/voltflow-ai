! ============================================================================
! THERMAL SOLVER FORTRAN - Version 2.3
...
subroutine initialize_solver()
    print *, "========================================"
    print *, "THERMAL SOLVER ND - Version 2.3"
    print *, "========================================"
        ...
        do k = 1, cfg%nz
            do j = 1, cfg%ny
                do i = 1, cfg%nx
                    ...
                    laplacian = (result%T(i+1,j,k) - 2._dp*result%T(i,j,k) + &
                                 result%T(i-1,j,k)) / (dx*dx)
                    if (cfg%ny > 1) then
                        laplacian = laplacian + (result%T(i,j+1,k) - &
                                    2._dp*result%T(i,j,k) + result%T(i,j-1,k)) / (dy*dy)
                    end if
                    if (cfg%nz > 1) then
                        laplacian = laplacian + (result%T(i,j,k+1) - &
                                    2._dp*result%T(i,j,k) + result%T(i,j,k-1)) / (dz*dz)
                    end if
...
program thermal_solver
    ...
    cfg%conductivity = 50._dp; cfg%density = 2700._dp; cfg%specific_heat = 900._dp
    cfg%initial_temp = 1000._dp; cfg%boundary_temp = 25._dp; cfg%heat_flux = 1000._dp
    cfg%nx = 100; cfg%ny = 1; cfg%nz = 1; cfg%max_iterations = 5000
    cfg%dt = 0._dp; cfg%tolerance = 1.e-_dp; cfg%geometry_type = '1d_rod'
    ...
    if (command_argument_count() > 0) then
        ...
        if (file_exists) then
            ...
            if (io_status == 0) then
                read(10, *) cfg%conductivity; read(10, *) cfg%density; read(10, *) cfg%specific_heat
                read(10, *) cfg%initial_temp; read(10, *) cfg%boundary_temp; read(10, *) cfg%heat_flux
                read(10, *) cfg%nx, cfg%ny, cfg%nz; read(10, *) cfg%max_iterations
                read(10, *) cfg%dt; read(10, *) cfg%tolerance
                read(10, '(A)') cfg%geometry_type; read(10, '(A)') cfg%output_file
                close(10)
            end if
        end if
    end if

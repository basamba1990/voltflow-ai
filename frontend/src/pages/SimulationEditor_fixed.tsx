...
export default function SimulationEditor() {
  ...
  // 🔥 FIX NON-BLOQUANT : Gestion de l'upload de fichier
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Appel du service (maintenant non-bloquant pour l'analyse)
      ...
      // Lancement automatique si ID disponible
  ...
  }, [id, user, isRunning, startSimulationHook, refresh]);
          ...
          <CardContent className="flex-1 p-0 relative">
            {prepareViewerData ? (
              <>
                ...
                {/* Statistiques de Simulation corrigées */}
                    ...
                    <CardContent className="p-3 pt-2 space-y-1">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-sm font-mono font-bold text-blue-500">
                          {(results?.min_temperature ?? results?.temperature_stats?.min ?? 0).toFixed(1)}°C
                        </span>
                      </div>
                      <div className="flex justify-between items-center border-t pt-1 mt-1">
                        <span className="text-xs">Moyenne</span>
                        <span className="text-sm font-mono font-bold">
                          {(results?.average_temperature ?? results?.temperature_stats?.avg ?? 0).toFixed(1)}°C
                        </span>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card className="bg-background/80 backdrop-blur-sm border-none shadow-md w-48">
                    ...
                    <CardContent className="p-3 pt-2 space-y-1">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-sm font-mono font-bold">
                          {results?.iterations ?? results?.convergence_metrics?.iterations ?? '0'}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-xs">Résidu</span>
                        <span className="text-sm font-mono font-bold text-green-600">
                          {(results?.final_residual ?? results?.convergence_metrics?.final_residual ?? 0).toExponential(2)}
                        </span>
                      </div>
                    </CardContent>
                  </Card>
        ...
        <Card className="flex flex-col">
          ...
          <CardContent className="flex-1 overflow-y-auto space-y-6">
            ...
            <div className="grid gap-2">
              ...
              {formData.materialId && materialsData.find(m => m.id === formData.materialId) && (
                <div className="text-xs text-muted-foreground grid grid-cols-2 gap-1 mt-1">
                  <span>Conductivité: {materialsData.find(m => m.id === formData.materialId)?.conductivity} W/m·K</span>
                  <span>Densité: {materialsData.find(m => m.id === formData.materialId)?.density} kg/m³</span>
                </div>
              )}
            </div>

            <div className="grid gap-2">
              <Label htmlFor="meshDensity">Densité du Maillage</Label>
              <Select
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="medium">Moyen (équilibre)</SelectItem>
                  <SelectItem value="high">Élevée (précis)</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  type="number"
                  value={formData.nx}
                  onChange={(e) => setFormData(prev => ({ ...prev, nx: parseInt(e.target.value) || 1 }))}
                />
              </div>
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  type="number"
                  value={formData.ny}
                  onChange={(e) => setFormData(prev => ({ ...prev, ny: parseInt(e.target.value) || 1 }))}
                />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="nz">NZ</Label>
                <Input
                  id="nz"
                  type="number"
                  value={formData.nz}
                  onChange={(e) => setFormData(prev => ({ ...prev, nz: parseInt(e.target.value) || 1 }))}
                />
              </div>
            </div>
            ...
            <div className="grid gap-2">
              <Label htmlFor="solverType">Solveur</Label>
              <Select
                value={formData.solverType}
                onValueChange={(value) => handleSelectChange('solverType', value)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
